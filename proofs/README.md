...
stays below the extended significand width. If it passes the zero padding, the
normalized leading one remains in the reduced low slice, so sticky is already
one before any lower source bits can be lost and no all-discarded fallback is
needed.
